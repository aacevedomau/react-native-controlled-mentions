...
test("generates correct parts length from value", () => {
  expect(parseValue("Hey David! How are you?", partTypes).parts.length).toEqual(
    1
  );
  ...
  expect(
    parseValue("https://google.com/, Hey, @[David](1)! How are you?", partTypes)
      .parts.length
  ).toEqual(4);
  ...
  expect(
    parseValue("#[help](help), @[David](1), #[to_do](to_do)!", partTypes).parts
      .length
  ).toEqual(6);
});
...
test("getting correct mention part type keywords", () => {
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 0, end: 0 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": undefined });
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 7, end: 7 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": "" });
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 12, end: 12 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": "David" });
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 19, end: 19 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": "David Tabaka" });
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 20, end: 20 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": undefined });
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 10, end: 10 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": undefined });
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 15, end: 15 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": undefined });
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 17, end: 17 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": undefined });
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 1, end: 1 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": "" });
  ...
  expect(
    getMentionPartSuggestionKeywords(parts, plainText, { start: 3, end: 3 }, [
      mentionPartType,
    ])
  ).toEqual({ "@": "" });
});
...
test("replacing mention's value", () => {
  ...
  const replacedByTitle = replaceMentionValues(
    value,
    ({ title }) => `@${title}`
  );
  expect(replacedByTitle).toEqual<string>("@David and @Mary");
});
